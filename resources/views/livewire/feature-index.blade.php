<div>
    <header class="bg-gray-50 shadow-lg border">
        ...
        <input wire:model.live="search" type="text" placeholder="Search features..." class="border rounded px-3 py-1 mb-4">
        </div>
    </header>
                ...
                <a href="{{ route('features.show', $feature->id) }}" class="block bg-white p-6 rounded shadow hover:bg-gray-50">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">ID: {{ $feature->id }}</p>
                        </div>

                        @php
                            $typeCounts = $feature->issues->groupBy('type')->map->count();
